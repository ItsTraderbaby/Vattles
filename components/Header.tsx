...
const Header: React.FC<HeaderProps> = ({ userProfile, onNavigate, onOpenCreateVattle, onLogout }) => {
      ...
      <div className="max-w-screen-2xl mx-auto flex justify-between items-center">
        <div className="flex items-center gap-6">
          ...
          <nav role="navigation" className="hidden md:flex items-center gap-6 text-sm font-semibold text-gray-300">
            <button onClick={() => onNavigate('rankings')} className="hover:text-white transition-colors flex items-center gap-2"><TrophyIcon className="h-4 w-4" aria-hidden="true"/> Rankings</button>
            <button onClick={() => onNavigate('tournaments')} className="hover:text-white transition-colors flex items-center gap-2"><UserGroupIcon className="h-4 w-4" aria-hidden="true"/> Tournaments</button>
        ...
        <div className="flex items-center gap-4">
          <button onClick={onOpenCreateVattle} className="px-4 py-2 font-semibold rounded-lg text-sm transition-all bg-purple-600 hover:bg-purple-500 text-white shadow-lg shadow-purple-600/30">
            Create Vattle
          </button>
