...
const VotingView: React.FC<VotingViewProps> = ({ vattle, onVote, hasVoted, onBack, userProfile, onEndorse, onCloneToLab }) => {
  ...
  const appA = { ...mockAppA, title: isCompleted ? `${vattle.creatorName}'s Submission` : mockAppA.title };
  const appB = { ...mockAppB, title: isCompleted ? `${vattle.invitedOpponent}'s Submission` : mockAppB.title };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <AppPanel 
                    submission={appB}
                    mode={isCompleted ? 'vod' : 'voting'} 
                    isWinner={vattle.winner === vattle.invitedOpponent}
                    ratings={ratingsB} 
                    onRate={handleRateB} 
                    onVote={() => handleVoteClick('B')} 
                    hasVoted={hasVoted} 
                />
                {isCompleted && <EndorsementSection participantName={vattle.invitedOpponent} />}
            </div>
        </div>
